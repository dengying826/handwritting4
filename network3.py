...
def plot_image(image, x, j):
    plt.imshow(image.reshape(28,28),cmap='binary')
    plt.savefig('image/hidden1_n{}_{}.png'.format(x, j))
    # plt.show()
    ...
    def train(self, training_data, test_data, epochs, batch_size, learning_rate,
        ...
        for j in range(epochs):
            for index in range(10):
                plot_image(self.weights[0][index], index, j)
        ...
        x = np.arange(epochs)
        if monitor_test_cost:
            y1 = np.array(test_cost)
            plt.plot(x, y1, color='Green')
            plt.xlabel('epochs')
            plt.ylabel('test_cost')
            plt.show()
        if monitor_test_accuracy:
            ...
            plt.show()
        if monitor_training_cost:
            y3 = np.array(training_cost)
            plt.plot(x, y3, color='Green')
            plt.xlabel('epochs')
            plt.ylabel('training_cost')
            plt.show()
        if monitor_training_accuracy:
            y4 = np.array(training_accuracy)
...
if __name__ == '__main__':
    # 加载训练集与测试集
    traning_data, test_data = load_data('data/mnist.pkl.gz')
    # 初始化网络
    network = Network([784,300,100,10],cost = CrossEntropyCost)
    # 训练
    network.train(traning_data,test_data, 200, 32, 1,
